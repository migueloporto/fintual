class Portfolio{
    constructor(){
        
    }

    getPriceStocks(date){
        let url = 'http://api.marketstack.com/v1/eod?access_key=c40428f9b00f27522b9e09f2ce2bc806&symbols=TSLA&date_from=2021-11-01&date_to=2021-11-01';
        fetch(url)
        .then(resp=>JSON.parse(resp))
        .then(stock=>{
            console.log('stock', stock);
            return stock[0].close;
        })
    }

    getStocks(){
        let stocks = [
            {name:'TSLA', price:this.getPriceStocks},
            {name:'NVDA', price:this.getPriceStocks},
            {name:'AAPL', price:this.getPriceStocks},
            {name:'AMZN', price:this.getPriceStocks},
            {name:'AMD', price:this.getPriceStocks}
        ];

        return stocks;
    };

    profit(dateIni, dateEnd){
        return new Promise((resolve)=>{
            let stocks = this.getStocks(),
                totalIni = 0,
                totalEnd = 0,
                promisesIni = [],
                promisesEnd = [];

            stocks.forEach(stock=>{
                promisesIni.push(stock.price(dateIni));
                promisesEnd.push(stock.price(dateEnd));
            });

            Promise.all(promisesIni)
            .then(values=>{
                values.forEach(value=>{
                    totalIni+=value;
                });

                Promise.all(promisesEnd)
                .then(values=>{
                    values.forEach(value=>{
                        totalEnd+=value;
                    });

                    resolve({
                        profit:totalIni - promisesEnd
                    })
                });
            });
        });
    }
}

let portfolio = new Portfolio(),
    dateIni = momment('2021-02-05').format('YYYY-MM-DD'),
    dateEnd = momment('2021-11-30').format('YYYY-MM-DD'),
    resp = portfolio.profit(dateIni, dateEnd)
            .then((resp)=>{
                console.log('resp', resp);
            });